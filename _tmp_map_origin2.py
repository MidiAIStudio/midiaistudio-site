...
best = (1e9, None)
for sx100 in range(860, 930, 2):
    sx = sx100 / 1000.0
    for sy100 in range(520, 590, 2):
        sy = sy100 / 1000.0
        for oy in range(150, 172):
            bt_est = oy + yt * sy
            if bt_est < 168 or bt_est > 188:
                continue
            for ox in range(110, 150, 2):
...
pad = 4
bl, bt, br, bb = max(0, bl-pad), max(0, bt-pad), min(Bw-1, br+pad), min(Bh-1, bb+pad)
...
l,t,w,h = bl/1280*100, bt/720*100, (br-bl+1)/1280*100, (bb-bt+1)/720*100
print("CSS:", "style=\"--l:%.2f%%;--t:%.2f%%;--w:%.2f%%;--h:%.2f%%\"" % (l,t,w,h))
